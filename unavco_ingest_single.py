...
url_prefix = "http://web-services.unavco.org/gps/data/position/"
url_suffix = "/v1?referenceFrame=igs08&starttime=1980-01-06T00:00:00&endtime=" + today
...
for entry in station_list:
    ...
    last_item = ""
    for item in records:
        [ctimestamp, north, east, up, nsig, esig, usig, quality] = [x.strip() for x in item.split(',')]
	if abs(float(north)) > 20*1000 or abs(float(east)) > 20*1000 or abs(float(up)) > 20*1000:
            continue
        ...
        if last_item == "":
            ...
            [ltimestamp, lnorth, least, lup, lnsig, lesig, lusig, lquality] = [x.strip() for x in last_item.split(',')]
